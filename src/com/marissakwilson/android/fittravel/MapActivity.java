...
public class MapActivity extends FragmentActivity implements
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (savedInstanceState == null) {
			...
			Location.distanceBetween(mTrip.getLocationA()[0],
					mTrip.getLocationA()[1], mTrip.getLocationB()[0],
					mTrip.getLocationB()[1], results);
			mTrip.setTotalDistance();
			mTrip.setCurrentDistance(4750000);
		}
		
		//TODO Fix this
//		else{
//			mTrip = savedInstanceState.get(trip);
//		}
...
//		googleMap.setMyLocationEnabled(true);
		...
		googleMap.addMarker(new MarkerOptions().position(locationA).title(
				"Starting point"));
		googleMap.addMarker(new MarkerOptions().position(locationB).title(
				"Ending point"));
		...
		Polyline bottomLine = googleMap.addPolyline(new PolylineOptions()
				.add(locationA, locationB).width(10).color(Color.BLUE)
				.geodesic(true));

		Polyline topLine = googleMap.addPolyline(new PolylineOptions()
				.add(locationA, midPoint).width(10).color(Color.GREEN)
				.geodesic(true));

		String progressTitle = mTrip.getCurrentDistance() + "/" +mTrip.getTotalDistance();
		googleMap.addMarker(new MarkerOptions().position(midPoint).title(progressTitle).icon(BitmapDescriptorFactory.fromResource(R.drawable.ic_walk)));
	...
	public void getLastLocation() {
		...
		if (mLastLocation != null) {
			mTrip.setLocationA(mLastLocation.getLatitude(),
	...
	public LatLng getProgress() {
		...
		double lngA = tmpA[1];
		double lngB = tmpB[1];
		double percentage = mTrip.getCurrentDistance()
				/ mTrip.getTotalDistance();
		...
		// Defining Cartesian components of the starting and ending points as
		// vectors
